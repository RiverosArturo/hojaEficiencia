    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
        ...
        <input type="text" value={codigoTabla} onChange={handleCodigoChange}
          placeholder="Escribe o selecciona un código" className="w-full p-2 border rounded mb-2" />

        {sugerencias.length > 0 && (
          <ul className="bg-white border rounded shadow max-h-40 overflow-auto mb-4">
            {sugerencias.map((codigo) => (
              <li key={codigo} className="p-2 hover:bg-gray-200 cursor-pointer" onClick={() =>
        ...
        <label className="block text-gray-700 font-medium">O seleccionar tabla:</label>
        ...
        <label className="block text-gray-700 font-medium">Ingresar números o rangos:</label>
        <input type="text" value={numerosInput} onChange={(e) => setNumerosInput(e.target.value)}
          placeholder="Ej: 1-9,37-41,47" className="w-full p-2 border rounded mb-4" />
        <button onClick={handleCalcular}
          className="w-full bg-blue-500 text-white p-2 rounded hover:bg-blue-600 transition">
          Calcular
        </button>
        <h2 className="text-xl font-bold mt-4 text-center">Total: {resultado}</h2>
        <h2 className="text-xl font-bold mt-4 text-center">Porcentaje: {total}%</h2>
      </div>
    </div>
  );
}